...
function BulkOperations() {
  ...
  useEffect(() => {
    const fetchSystemData = async () => {
      try {
        const [shiftsRes, doorsRes, usersRes] = await Promise.all([
          api.get('/shifts'),
          api.get('/doors'),
          api.get('/users') 
        ]);
  ...
  const handleSearchInputChange = (e) => {
    ...
    if (value.length >= 1) {
      ...
      allUsers.forEach(user => {
        ...
        if (key && key.toLocaleLowerCase('tr-TR').includes(lowerVal)) { 
      ...
      const suggestionList = Object.keys(grouped).map(key => ({
        label: key, // Bileşen "label" okur
        subLabel: 'Örnek: ' + (grouped[key].slice(0, 2).join(', ') + (grouped[key].length > 2 ? '...' : '')), // Bileşen "subLabel" okur
        value: key // Bileşen "value" okur
      }));

      if (suggestionList.length === 0) {
        suggestionList.push({ label: value, subLabel: 'Sistemde eşleşen kayıt bulunamadı', value: value });
      }
  ...
  const handleDoorSelection = (doorId) => {
    setSelectedDoors(prev => prev.includes(doorId) ? prev.filter(id => id !== doorId) : [...prev, doorId]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const hedefMetni = hedefTuru === 'Tumu' ? 'SİSTEMDEKİ TÜM AKTİF' : `"${hedefDeger}" isimli ${hedefTuru === 'Departman' ? 'departmandaki' : 'şirketteki/taşerondaki'} TÜM`;
    ...
    if (window.confirm(uyariMesaji)) {
      ...
      try {
        ...
        if (islemTuru === 'vardiya') {
          res = await api.post('/users/bulk/shift', { hedef_turu: hedefTuru, hedef_deger: hedefDeger.trim(), vardiya_id: vardiyaId });
          setVardiyaId('');
        }
        else if (islemTuru === 'yetki') {
          res = await api.post('/users/bulk/doors', { hedef_turu: hedefTuru, hedef_deger: hedefDeger.trim(), doorIds: selectedDoors });
          setSelectedDoors([]);
        }
        else if (islemTuru === 'durum') {
          res = await api.post('/users/bulk/status', { hedef_turu: hedefTuru, hedef_deger: hedefDeger.trim(), durum: durumHedefi, cikis_tarihi: exitData.cikis_tarihi, cikis_nedeni: exitData.cikis_nedeni });
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select 
                  ...
                  value={hedefTuru} 
                  onChange={(e) => { setHedefTuru(e.target.value); setHedefDeger(''); setSuggestions([]); }}
                >
          ...
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-100 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              <div>
                ...
                <select className="w-full px-4 py-3 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-white" value={islemTuru} onChange={(e) => setIslemTuru(e.target.value)}>
              ...
              {islemTuru === 'vardiya' && (
                <div className="animate-fade-in-up">
                  ...
                  <select className="w-full px-4 py-3 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-white" value={vardiyaId} onChange={(e) => setVardiyaId(e.target.value)}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-white p-4 border border-slate-200 rounded-xl max-h-60 overflow-y-auto">
                    {allDoors.map(door => (
                      <label key={door.ID} className="flex items-center p-2 hover:bg-slate-50 cursor-pointer rounded transition-colors border border-transparent hover:border-slate-200">
                        <input type="checkbox" className="w-5 h-5 text-purple-600 rounded border-gray-300 focus:ring-purple-500" checked={selectedDoors.includes(door.ID)} onChange={() => handleDoorSelection(door.ID)} />
              ...
              {islemTuru === 'durum' && (
                <div className="animate-fade-in-up space-y-4">
                  <div>
                    ...
                    <select className="w-full px-4 py-3 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-red-500 bg-white text-red-600 font-bold" value={durumHedefi} onChange={(e) => setDurumHedefi(Number(e.target.value))}>
                  ...
                  {durumHedefi === 0 && (
                    <div className="grid grid-cols-1 gap-4 bg-red-50 p-4 rounded-xl border border-red-100">
                      <div><label className="block text-xs font-bold text-slate-700 mb-1">Toplu Çıkış Tarihi *</label><input type="date" value={exitData.cikis_tarihi} onChange={(e) => setExitData({...exitData, cikis_tarihi: e.target.value})} required className="w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-red-500" /></div>
                      <div><label className="block text-xs font-bold text-slate-700 mb-1">Toplu Çıkış Nedeni (Opsiyonel)</label><textarea rows="2" value={exitData.cikis_nedeni} onChange={(e) => setExitData({...exitData, cikis_nedeni: e.target.value})} placeholder="Örn: Proje Bitişi" className="w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-red-500 resize-none"></textarea></div>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="flex justify-end pt-4 border-t border-slate-200">
            <button type="submit" disabled={loading} className={`px-8 py-4 text-white font-bold text-lg rounded-xl transition-colors shadow-lg disabled:opacity-50 ${islemTuru === 'durum' && durumHedefi === 0 ? 'bg-red-600 hover:bg-red-700' : 'bg-slate-900 hover:bg-slate-800'}`}>
